...
The dataset used and proposed by [VJO2011](http://www.inf.ufpr.br/lesoliveira/download/ijcnn2011.pdf) contains 100 different authors whose texts were uniformly distributed over 10 different subjects: Miscellaneous, Law, Economics, Sports, Gas- tronomy, Literature, Politics, Health, Technology, and Tourism. In this database all the subjects have ten different authors.
...
At first it was create a csv file to store all the text from the initial dataset provided by the original work. The dataset was distribuited in folders organized by subjects and authors  in folder `data/raw/BASE DE DADOS - PAULO JR VARELA`. In notebook `notebooks/00-Make-Dataset.ipynb` this csv file is built and stored at folder `data` as `data_raw.csv`.

### Exploratory Visualization

It was verified that the authors and subject distribuitions were in accordance to the reported by the original work as it's shown in the figures below:
...
It's importante to notice the normal shape distribuition of `average word length`:
...
Then, I generate the wordcloud for each of the subjects sinche there are many authors. In this approach, the wordcloud was generated based on the most relevant words in each subject by using [TF-IDF](https://en.wikipedia.org/wiki/Tf%E2%80%93idf) feature extractions technique:
...
Several algorithms and techniques were used and applied based on literacture on NLP classification problems. Most of then were used without any hyperparameter tunning.
...
For this project, I'll use as a benchmark model the accuracy of the work in [VJO2011](http://www.inf.ufpr.br/lesoliveira/download/ijcnn2011.pdf) e [OOJ2013](http://www.inf.ufpr.br/lesoliveira/download/FSI2013.pdf), which were a 74% and 77% using with a SVM classifier + a feature selection using a multi-objective algorithm and compression models approach, respectively.
...
Here, the models were implementend using the framework [Keras](https://keras.io/).
...
My best accuracy was 0.84 on the overfitted model and 0.72 in a less overfitted model. The orignal result was 0.74.

The solution presented is more like a baseline for future work than a proper solution. As the original work was more intended to show a new approach then to provide a solution to the problem since the dataset is very small.

## V. Conclusion

### Free-Form Visualization

Here I'd like to plot the graph of the model selection that I borrowed from the course and it showed very useful. I'd like to make this graph with all models built, but it was more difficult that I imagine. This plot helped me to find the better classical machine learning algorith for the problem and to analise it in a much faster way.
...
The most challenge thing about this project was to learn the basics [Keras](https://keras.io/) and FastText, so I could use them. The other difficult thing was the lack of data, this dataset is very small.

As my final thoughts, I think the simpler approach resulted in a satisfatory solution to the problem and it was better than the benchmark result.

### Improvement

I think that the best model could be improved by building a ensemble model to balanced the miss classified authors. It could be made a more deep analysis on the most miss classified text to seek for insights, like words that could be generating this errors.
...
Alse I could build a topic model using [LDA - Latent Dirichlet allocation](https://en.wikipedia.org/wiki/Latent_Dirichlet_allocation) and use it as feature to improve the classification performance.
